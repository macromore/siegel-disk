...
yMax = 1;
step = .005;
xSteps = (xMax-xMin)/step + 1;
ySteps = (yMax-yMin)/step + 1;
juliaColor = zeros(xSteps,ySteps);
for j = 1:xSteps
    for k = 1:ySteps
        juliaColor(j,k) = (j-1)*step + xMin + ((k-1)*step + yMin)*1i;
    end
end
for j = 1:maxIter
    juliaColor = juliaColor .* juliaColor + a*juliaColor;
end
surf(abs(juliaColor)')

% contour(log(log(log((juliaColor)+1)+1))', 'Fill','on')
xlab = xMin:.5:xMax;
...
for j = 2:maxPower % j will control the attempted step size
    ...
    while iter <= maxIter % Repeatedly try larger h_1 values
        ...
        if normF > tolerance || mag > tolerance % Backtrack on failure
            ...
            plot((real(Pz(:,iter))-xMin)/step,(imag(Pz(:,iter))-yMin)/step,'b');
            figure(1)
        end % if
    end % while loop
end % for loop
